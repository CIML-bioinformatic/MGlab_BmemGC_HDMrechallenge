...
# Split hto assays for mice eand tissue in 2 sets to perform double demux
hto_assay_mice   = subset(hto_assay, feature = grep("M[0-9]", rownames(hto_assay), value = TRUE))
hto_assay_tissue = subset(hto_assay, feature = grep("M[0-9]", rownames(hto_assay), value = TRUE, invert = TRUE))

sc10x[["HTOMICE"]]   = hto_assay_mice
sc10x[["HTOTISSUE"]] = hto_assay_tissue
...
sc10x = NormalizeData(sc10x, assay = "HTOMICE", normalization.method = "CLR")

# Demultiplex cells based on HTOs
sc10x = HTODemux(sc10x, assay = "HTOMICE", positive.quantile = 0.99)

# Group cells based on the max HTO signal and show ridgeplot
#Idents(sc10x) = "HTO_maxID"
RidgePlot(sc10x, assay = "HTOMICE", features = rownames(sc10x[["HTOMICE"]]), ncol = 2)
kable(table(sc10x[["HTOMICE_classification.global"]]), col.names = NULL)

# Plot UMIs/genes distribution for singlets doublets and negative cells
#Idents(sc10x) <- "HTOMICE_classification.global"
#VlnPlot(sc10x, features = "nCount_RNA", pt.size = 0.1, log = TRUE)
#VlnPlot(sc10x, features = "nFeature_RNA", pt.size = 0.1, log = TRUE)

#Idents(sc10x) = "HTOMICE_maxID"
#VlnPlot(sc10x, features = "nCount_RNA", pt.size = 0.1, log = TRUE)
#Idents(sc10x) = "HTOMICE_classification"
#VlnPlot(sc10x, features = "nCount_RNA", pt.size = 0.1, log = TRUE)

## @knitr demuxHTOMICE_tsne

# Calculate tSNE embedding of the HTO data
DefaultAssay(sc10x) = "HTOMICE"
...
## @knitr demuxHTOTISSUE

# Normalize HTO counts
sc10x = NormalizeData(sc10x, assay = "HTOTISSUE", normalization.method = "CLR")

# Demultiplex cells based on HTOs
sc10x = HTODemux(sc10x, assay = "HTOTISSUE", positive.quantile = 0.99)

# Group cells based on the max HTO signal and show ridgeplot
#Idents(sc10x) = "HTO_maxID"
RidgePlot(sc10x, assay = "HTOTISSUE", features = rownames(sc10x[["HTOTISSUE"]]), ncol = 2)
kable(table(sc10x[["HTOTISSUE_classification.global"]]), col.names = NULL)
...
## @knitr demuxHTOTISSUE_tsne

# Calculate tSNE embedding of the HTO data
DefaultAssay(sc10x) = "HTOTISSUE"
sc10x = ScaleData(sc10x, features = rownames(sc10x), verbose = FALSE)
sc10x = RunPCA(sc10x, features = rownames(sc10x), approx = FALSE, verbose = FALSE)
sc10x = RunTSNE(sc10x, dims = 1:20, check_duplicates = FALSE)

# Plot showing singlet/doublet/negative cells
Idents(sc10x) <- "HTOTISSUE_classification.global"
DimPlot(sc10x, reduction = "tsne")

# Show affectation (with all doublets combination)
Idents(sc10x) <- "HTOTISSUE_classification"
DimPlot(sc10x, reduction = "tsne")

## @knitr demuxHTOTISSUE_tsne_final
...
## @knitr demuxHTO_summary

# Set expected slot 'HTO_classification' to the combination of both HTOs
sc10x[["HTO_classification"]] = paste(sc10x[["HTOTISSUE_classification", drop = TRUE]], sc10x[["HTOMICE_classification", drop = TRUE]], sep = "_")
