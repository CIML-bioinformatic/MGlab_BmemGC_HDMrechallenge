...
#############################################
# VIEW OUTLIERS ON PREVIOUSLY PROCESSED DATA
#############################################

# ..............................................................................
## @knitr outliers_on_PCA
# ..............................................................................
# Possible to predefine 'currentTitleDepth' in Rmd to get a crrect title level
# (defaults to 2)

# Create a categorical metadata to classify outliers
#sc10x[["outlierCategory"]] = 

# Get the various outliers columns to iterate over them in figures  
metaColnamesOutliers = sort(grep("outlier", colnames(sc10x[[]]), value = TRUE))

cat('\n \n')

for(currentMeta in metaColnamesOutliers)
{
  
  cat(paste(paste(rep("#", ifelse(exists("currentTitleDepth"), currentTitleDepth+1, 2)), collapse = ""), currentMeta, "\n\n"))
  
  # Same PCA plots but highlight UMI counts
  invisible( apply( combn( 1:PCA_PLOTS_NBDIMS, 2), 2, function(dims)
  {
    print( DimPlot( sc10x, group.by = currentMeta, reduction = "pca", dims = dims) +
...
for(currentMeta in metaColnamesOutliers)
{
  ...
  print( DimPlot( sc10x, group.by = currentMeta, reduction = "umap") +
           theme( #legend.position = "none",
             plot.margin = margin(0, 0, 0, 0, "cm")) +
           ggtitle(currentMeta));
...
# ..............................................................................
## @knitr outliers_on_dimreduc_bycluster
# ..............................................................................
# Possible to predefine 'currentTitleDepth' in Rmd to get a crrect title level
# (defaults to 2)
...
current_embeddings = Embeddings(sc10x, reduction = "umap")
...
#  # Using facetting
# 
#   ggplot(cbind(as.data.frame(current_embeddings), 
#                sc10x[[metaColnamesOutliers]],
#                Cluster = Idents(sc10x)), 
#          aes_string(x = colnames(current_embeddings)[1], 
#                     y = colnames(current_embeddings)[2],
#                     color = currentMeta),
#          color = currentMeta) +
#     geom_point() +
...
#   
  ...
  # Loop over clusters to highlight outliers in correpsonding cells 
  for(currentCluster in levels(Idents(sc10x)))
  {
    print( ggplot(cbind(as.data.frame(current_embeddings), 
                        Coloring = ifelse(as.numeric(as.character(Idents(sc10x)))==currentCluster, sc10x[[currentMeta, drop = TRUE]], NA), # Define a column used for coloring ignoring cells not in selected cluster 
                        Cluster = Idents(sc10x)), 
                  aes_string(x = colnames(current_embeddings)[1], 
                             y = colnames(current_embeddings)[2],
                             color = "Coloring")) +
             geom_point(alpha=0.3) +
             ggtitle(paste0(currentMeta, " - Cluster: ", currentCluster)))
...
for(currentMeta in metaColnamesOutliers)
{
  ...
  print( kable( addmargins(table(cbind(sc10x[[currentMeta]], 
                                       Idents(sc10x))),
                           margin = c(1,2)),
                caption = currentMeta) %>%
...
########################################################################
# REMOVE OUTLIERS TO REPROCESS DATA (for comparison with previous step)
########################################################################

# ..............................................................................
## @knitr outliers_removal
# ..............................................................................

outliers = sc10x[[ "outlier", drop=TRUE ]]
print(sc10x)
cat('\n \n')

cat(paste0("Removing : ", sum(outliers), " cells"))
cat('\n \n')
