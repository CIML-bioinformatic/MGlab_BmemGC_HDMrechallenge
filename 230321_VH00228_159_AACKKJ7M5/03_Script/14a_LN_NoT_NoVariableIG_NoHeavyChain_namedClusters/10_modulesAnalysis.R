...
## @knitr heterogeneity_modules_heatmap_byCluster
...
modulesScoreMatByCluster = apply(modulesScoreMat, 1 , tapply, INDEX = Idents( sc10x), mean)
...
# Plot the 'non-interactive' heatmap
pheatmap( modulesScoreMatByCluster,
          color = colorRampPalette( rev( RColorBrewer::brewer.pal(n = 7, name = "RdBu")))(100),
          cluster_rows = FALSE,
          cluster_cols = FALSE,
          scale = "row");
...
DotPlot(sc10x, features = paste0(names( MODULES_GENES),1), cols = "RdBu") + 
  coord_flip() +
...
invisible( lapply( paste0(names(MODULES_GENES), 1), violinFeatureByCluster, seuratObject = sc10x, clustersColor = clustersColor, yLabel = "Score", addStats = FALSE, trimTitle = 1, symmetricY=TRUE));
