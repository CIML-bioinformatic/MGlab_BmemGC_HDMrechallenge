...
# Remove cells with specifi tag in HTO (mostly to remove "negative" demux cells)
if(!is.null(FILTER_CELLS_HTO_REGEX))
{
  cat( "\n<br><br>Filtering HTO with regex '", FILTER_CELLS_HTO_REGEX, "' before further analyses...")
  cellsToRemove = cellsToRemove | grepl(FILTER_CELLS_HTO_REGEX, sc10x[["factorHTO", drop = TRUE]])
}
...
if(any( cellsToRemove))
{
  ...
  # Calling factor to recompute levels
  if(is.null( ORDER_CLUSTER)) 
  {
    Idents( sc10x) = factor( Idents( sc10x)) 
  } else
  {
    Idents( sc10x) = factor( Idents( sc10x), levels = ORDER_CLUSTER)
  }
