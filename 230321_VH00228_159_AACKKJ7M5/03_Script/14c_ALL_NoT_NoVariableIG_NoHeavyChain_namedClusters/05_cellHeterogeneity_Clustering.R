...
# Also compute by tissue (for chi2 analysis)
clustersCountByTissue = table(cbind(Idents( sc10x), data.frame(Tissue = gsub("_.*", "", sc10x[["factorHTO", drop = TRUE]]))))

cells_M456 = gsub(".*_", "", sc10x[["factorHTO", drop = TRUE]]) %in% c("M4", "M5", "M6")
clustersCountByTissue_M456 = table(cbind(factor(Idents( sc10x)[cells_M456]), data.frame(Tissue = gsub("_.*", "", factor(sc10x[["factorHTO", drop = TRUE]][cells_M456])))))
