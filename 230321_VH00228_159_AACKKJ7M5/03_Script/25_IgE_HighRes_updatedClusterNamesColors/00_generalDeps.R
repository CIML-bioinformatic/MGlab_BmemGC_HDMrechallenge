...
#library( parcats)       # interactive alluvial plots
...
violinFeatureByCluster = function(currentFeature, seuratObject, slot = "data", yJitter = 0.15, clustersColor = NULL, yLabel = "Norm. counts", addStats = TRUE, addTitle = TRUE, trimTitle = FALSE, symmetricY = FALSE) 
{
  ...
  ggFigure = ggplot( data.frame( Counts = featureValues, Cluster = Idents( seuratObject)), 
    ...
    geom_jitter( width = 0.2, height = yJitter, size = 0.4, colour = "#44444444") +
    geom_violin( aes( col = Cluster, fill = Cluster), scale = "width", alpha = 0.4, draw_quantiles = 0.5) +
    theme_minimal() +
    ylab( label = yLabel) +
    scale_x_discrete(guide = guide_axis(n.dodge = 2)) + # Dodge x axis labels
  ...
  # Create a symmetric Y axis (for modulescore)
  if(symmetricY)
  {
    ggFigure = ggFigure + geom_blank(aes(y = featureValues * -1))
  } 
